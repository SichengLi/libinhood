...
class Robinhood:
    ...
    def watchlists(self): 
        ...
        """
        return self.session.get(endpoints.watchlists() + 'gg/', timeout=15).json()
    ...
    def get_stock_info(self, url=None):
        res = self.session.get(endpoints.stock_info(url), params= None, timeout=15)
        res.raise_for_status()
        res = res.json()
    ...
    def my_place_stop_loss_sell_order(self,
                                   instrument_URL=None,
                                   symbol=None,
                                   time_in_force=None,
                                   stop_price=None,
        ...
        """Wrapper for placing stop loss sell orders
            ...
            Returns:
                (:obj:`requests.request`): result from `orders` put command
        """
        return(self.my_submit_sell_order(order_type='market',
                                 trigger='stop',
                                 side='sell',
                                 instrument_URL=instrument_URL,
                                 symbol=symbol,
    ...
    def my_submit_sell_order(self,
        ...
        if (price is not None):
            ...
            if (stop_price is not None):
                ...
                price = float(1)  # default to current bid price
